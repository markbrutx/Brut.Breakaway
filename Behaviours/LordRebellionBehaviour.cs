...
namespace Brut.Breakaway.Behaviours.Behaviours
{
	public class LordRebellionBehaviour : CampaignBehaviorBase
	{
		...
		private void OnDailyTickClan(Clan clan)
		{
			...
			{
				if (!BreakawayConfig.Settings.LordRebellionsEnabled)
				{
					return;
				}
				...
				var hasEnoughFiefs = (kingdomFiefs > 0 &&
					((BreakawayConfig.Settings.AverageAmountOfKingdomFiefsIsEnoughToRebel && clanFiefs >= (float)kingdomFiefs / kingdomClans) ||
					BreakawayConfig.Settings.MinimalAmountOfKingdomFiefsToRebel <= clanFiefs));
				var rebelRightNow = BreakawayConfig.Settings.DailyLordRebellionChance >= 1 ||
					(MBRandom.RandomFloat <= BreakawayConfig.Settings.DailyLordRebellionChance);
			...
			else
			{
				if (kingdom.Clans.Where(x => x.Leader.IsAlive).Count() == 1) // kingdom has the only one ruler clan
				{
					...
					else // look for ally
					{
						...
						if (BreakawayConfig.Settings.AllowUnions && LookForUnion(clan))
						{
							return;
						}
						...
						if (allyClan != null)
						{
							...
							int relationChange = BreakawayConfig.Settings.RelationChangeRulerWithSupporter;
							if (relationChange != 0)
							{
