...
namespace Brut.Breakaway.Behaviours
{
	public class NationalRebellionBehaviour : CampaignBehaviorBase
	{
		...
		{
			if (!BreakawayConfig.Settings.NationalRebellionsEnabled)
			{
				return;
			}
			...
			{
				var rebelRightNow = BreakawayConfig.Settings.DailyNationalRebellionChance >= 1 ||
					(MBRandom.RandomFloat <= BreakawayConfig.Settings.DailyNationalRebellionChance);

				if (group.Count() >= BreakawayConfig.Settings.MinimalRequiredNumberOfNativeLords && rebelRightNow)
				{
		...
		private Kingdom GoRebelKingdom(IEnumerable<Clan> clans)
		{
			...
			int relationChange = BreakawayConfig.Settings.RelationChangeNationalRebellionClans;
			if (relationChange != 0)
			{
