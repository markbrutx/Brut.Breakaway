...
namespace Brut.Breakaway.Behaviours.Behaviours
{
	public class AnarchyRebellionBehaviour : CampaignBehaviorBase
	{
		...
		{
			if (!BreakawayConfig.Settings.AnarchyRebellionsEnabled)
			{
				return;
			}

			var clanFiefsAmount = clan.GetFiefsAmount();
			if (clanFiefsAmount < BreakawayConfig.Settings.CriticalAmountOfFiefsPerSingleClan) return;
			var anarchySettlements = clan.Settlements.Where(x => x.IsTown &&
				CampaignTime.Hours(x.LastVisitTimeOfOwner) + CampaignTime.Days(BreakawayConfig.Settings.NumberOfDaysAfterOwnerVisitToKeepOrder) < CampaignTime.Now).ToArray();
			...
			foreach (var settlement in anarchySettlements.OrderByDescending(x => x.Position.Distance(clan.FactionMidSettlement.Position)))
			{
				var newRulerClan = availableClans
					...
					.FirstOrDefault();
				var rebelRightNow = BreakawayConfig.Settings.DailyAnarchyRebellionChance >= 1 ||
					(MBRandom.RandomFloat <= BreakawayConfig.Settings.DailyAnarchyRebellionChance);

				if (newRulerClan != null && rebelRightNow)
				{
					var rebelSettlements = new List<Settlement>();
					rebelSettlements.Add(settlement);
					int bonusSettlements = (BreakawayConfig.Settings.BonusRebelFiefForHighTierClan && newRulerClan.Tier > 4) ? 1 : 0;
