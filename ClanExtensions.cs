	...
	public static class ClanExtensions
	{
		...
		public static Kingdom CreateKingdom(this Clan clan, Settlement capital, TextObject intro)
		{
			...
			if (kingdom == null)
			{
				...
				if (!BreakawayConfig.Settings.KeepRebelBannerColors)
				{
		...
		public static void ChangeKingdom(this Clan clan, Kingdom newKingdom, bool rebellion)
		{
			...
			if (newKingdom != null)
			{
				...
				if (rebellion)
				{
					...
					{
						int relationChange = BreakawayConfig.Settings.RelationChangeRebelWithRulerVassals;
						if (c.Leader == oldKingdom.Leader)
						{
							relationChange = BreakawayConfig.Settings.RelationChangeRebelWithRuler;
						}
						else if (c.Leader.IsFriend(oldKingdom.Leader))
						{
							relationChange = BreakawayConfig.Settings.RelationChangeRebelWithRulerFriendVassals;
						}
						else if (c.Leader.IsEnemy(oldKingdom.Leader))
						{
							relationChange = BreakawayConfig.Settings.RelationChangeRebelWithRulerEnemyVassals;
						}
				...
				{
					int relationChange = BreakawayConfig.Settings.RelationChangeUnitedRulers;
					if (relationChange != 0)
					{
